...
class MainValidator {
	...
	// ─────────────────────────────────────────────────────────────
	//  REST/steps-style JSON runner  (for USBank-style test files)
	// ─────────────────────────────────────────────────────────────
	@Keyword
	static void runRestJson(String calcId) {
		resetRunFlag()
		resetFailCount()
		prepareLogging(calcId)
		...
		Map firstCase = (root?.cases instanceof List && !root.cases.isEmpty())
			? (Map) root.cases[0] : (Map) root
	
		// Resolve globals
		Map<String,String> globals = [:]
		((firstCase?.globals ?: [:]) as Map).each { k, v -> globals[k.toString()] = v.toString() }
		String baseUrl = globals['BASEURL'] ?: gv('baseUrl', 'https://expert.leadfusion.com')
	
		List<Map> steps = (firstCase?.steps ?: []) as List<Map>
		logStep("Case: ${firstCase?.id ?: calcId}  |  Steps: ${steps.size()}")
	
		steps.each { Map step ->
			String stepName = step.name ?: '(unnamed)'
			String method   = (step.method ?: 'GET').toUpperCase()
			String endpoint = step.endpoint ?: ''
			Map<String,String> query  = (step.query  ?: [:]) as Map<String,String>
			Map<String,Object> expect = (step.expect ?: [:]) as Map<String,Object>
	
			// Resolve {{TOKEN}} placeholders in query values
			Map<String,String> resolvedQuery = [:]
			query.each { k, v ->
				String resolved = v.toString()
				globals.each { gk, gv2 -> resolved = resolved.replace("{{${gk}}}", gv2) }
				resolvedQuery[k] = resolved
			}
	
			// Build URL with query string
			String url = baseUrl + endpoint
			if (resolvedQuery) {
				String qs = resolvedQuery.collect { k, v ->
					"${URLEncoder.encode(k,'UTF-8')}=${URLEncoder.encode(v,'UTF-8')}"
				}.join('&')
				url = "${url}?${qs}"
			}
			logStep("STEP [${stepName}] ${method} ${url}")
			...
			logStep("STEP [${stepName}] Response status: ${statusCode}")
	
			// Assert status
			int expectedStatus = (expect.status ?: 200) as int
			if (statusCode != expectedStatus) {
				captureFailure(stepName, 'status', '', String.valueOf(expectedStatus), String.valueOf(statusCode))
				return  // skip body checks for this step
			}
	
			String body = response.getResponseText()
			logPreview("STEP_${stepName}", body)
	
			def json
			try { json = new JsonSlurper().parseText(body) }
			catch (Exception e) {
				captureFailure(stepName, 'responseBody', 'parse', '[valid JSON]', e.message)
				return
			}
	
			// notEmpty
			if (expect.containsKey('notEmpty')) {
				String path = expect.notEmpty.toString()
				def val = (path == '$') ? json : resolveJsonPath(json, path, stepName)
				boolean empty = (val == null || (val instanceof List && val.isEmpty())
											 || (val instanceof Map  && val.isEmpty())
											 || val.toString().trim().isEmpty())
				if (empty) captureFailure(stepName, path, 'notEmpty', '[not empty]', String.valueOf(val))
				else        logStep("· [${stepName}] notEmpty OK: ${path}")
			}
	
			// contains — [{path, value}, ...]
			((expect.contains ?: []) as List<Map>).each { Map c ->
				String path   = c.path?.toString()
				String expVal = c.value?.toString()
				List<Object> allVals = resolveJsonPathAll(json, path, stepName)
				boolean found = allVals.any { it?.toString() == expVal }
				if (!found) captureFailure(stepName, path, 'contains', expVal, allVals.join(', '))
				else         logStep("· [${stepName}] contains OK: ${path} ∋ '${expVal}'")
			}
	
			// equals — {path: value, ...}
			((expect.equals ?: [:]) as Map<String,Object>).each { String path, Object expVal ->
				def actual    = resolveJsonPath(json, path, stepName)
				String expStr = String.valueOf(expVal)
				String actStr = String.valueOf(actual)
				boolean match = (expStr == actStr)
				if (!match) {
					try { match = (new BigDecimal(expStr).compareTo(new BigDecimal(actStr)) == 0) }
					catch (Throwable ignore) {}
				}
				if (!match) captureFailure(stepName, path, 'equals', expStr, actStr)
				else         logStep("· [${stepName}] equals OK: ${path} = '${expVal}'")
			}
	
			// jsonTypes — {path: typeName, ...}
			((expect.jsonTypes ?: [:]) as Map<String,String>).each { String path, String expType ->
				def actual  = resolveJsonPath(json, path, stepName)
				String actType = classifyJsonType(actual)
				if (!expType.equalsIgnoreCase(actType))
					captureFailure(stepName, path, 'jsonType', expType, actType)
				else
					logStep("· [${stepName}] jsonType OK: ${path} is '${expType}'")
			}
		}
		...
		if (hasAnyFailure) {
			...
			logStep("✅ ${calcId} passed: all steps OK.")
		}
	}
	
	// Resolves $[0].field or $[*].field — returns first/single match
	private static Object resolveJsonPath(Object json, String path, String ctx) {
		try {
			if (path == '$') return json
			String p = path.startsWith('$') ? path.substring(1) : path
			Object cur = json
			def tokens = (p =~ /\[(\d+|\*)\]|\.([^.\[]+)/).collect { it }
			for (def tok : tokens) {
				String idx = tok[1]; String fld = tok[2]
				if (idx != null && !idx.isEmpty()) {
					if (idx == '*') return (cur instanceof List) ? cur : [cur]
					cur = (cur instanceof List) ? cur[Integer.parseInt(idx)] : null
				} else if (fld != null && !fld.isEmpty()) {
					cur = (cur instanceof Map) ? cur[fld] : null
				}
				if (cur == null) return null
			}
			return cur
		} catch (Throwable e) {
			logStep("⚠️ resolveJsonPath('${path}'): ${e.message}")
			return null
		}
	}
	
	// Resolves $[*].field -> flat list of all matching values
	private static List<Object> resolveJsonPathAll(Object json, String path, String ctx) {
		try {
			if (path == '$') return json instanceof List ? json : [json]
			String p = path.startsWith('$') ? path.substring(1) : path
			Object cur = json
			def tokens = (p =~ /\[(\d+|\*)\]|\.([^.\[]+)/).collect { it }
			for (int i = 0; i < tokens.size(); i++) {
				def tok = tokens[i]; String idx = tok[1]; String fld = tok[2]
				if (idx != null && !idx.isEmpty()) {
					if (idx == '*') {
						String remaining = tokens.drop(i + 1).collect { t ->
							t[1] != null ? "[${t[1]}]" : ".${t[2]}"
						}.join('')
						if (cur instanceof List) {
							return remaining
								? cur.collect { resolveJsonPath(it, '$' + remaining, ctx) }.findAll { it != null }
								: cur
						}
						return [cur]
					}
			...
			return cur instanceof List ? cur : [cur]
		} catch (Throwable e) {
			logStep("⚠️ resolveJsonPathAll('${path}'): ${e.message}")
			return []
		}
	}
	
	private static String classifyJsonType(Object val) {
		if (val == null)            return 'null'
		if (val instanceof Boolean) return 'boolean'
		if (val instanceof Integer || val instanceof Long || val instanceof Number) return 'number'
